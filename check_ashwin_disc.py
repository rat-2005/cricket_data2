...
matches = query(f"""
    ...
    cs_match AS (
        SELECT d.match_id, d.inning AS inningNumber,
            SUM(CASE WHEN d.wicket_type NOT IN ('run out', 'retired hurt', 'obstructing the field') THEN 1 ELSE 0 END) AS wickets
    ...
    WHERE COALESCE(ci.wickets, 0) != COALESCE(cs.wickets, 0)
""")
