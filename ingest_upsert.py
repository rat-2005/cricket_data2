    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if attempt == retries - 1:
                log.warning(f"Fetch totally failed after {retries} retries: {url} -> {e}")
        ...
        if status in (429, 502, 503, 504) or status is None:
            ...
            if attempt > 1:
                log.info(f"Rate limited or timeout on {url}. Retrying in {sleep_time}s (Attempt {attempt+1}/{retries})")
            await asyncio.sleep(sleep_time)

    # If all retries failed, log to unreachable file
    log.error(f"URL totally unreachable after {retries} retries: {url}")
...
async def process_match(pool, session, event_url):
    ...
    async with pool.acquire() as conn:
        # UPSERT STRATEGY: Delete the competition to cleanly overwrite all child data (innings, deliveries, matchcards)
        await conn.execute("DELETE FROM cricket.competitions WHERE id=$1", comp_id)
...
async def main():
    parser = argparse.ArgumentParser(description="Live/Upcoming Match Upsert")
    args = parser.parse_args()

    load_dotenv()
    DATABASE_URL = os.environ.get("DATABASE_URL")
    if not DATABASE_URL:
        log.error("DATABASE_URL is missing from .env")
        return
    ...
    try:
        # QUERY DATABASE FOR MATCHES THAT NEED UPDATING
        async with pool.acquire() as conn:
            records = await conn.fetch("""
                SELECT e.api_ref 
                FROM cricket.events e
                JOIN cricket.competitions c ON e.id = c.event_id
                JOIN cricket.match_status s ON c.id = s.competition_id
                WHERE s.state IN ('pre', 'in') 
            """)
            all_events = [r['api_ref'] for r in records if r['api_ref']]
            
        log.info(f"Found {len(all_events)} matches in 'pre' or 'in' state that need updating.")
        
        if not all_events:
            log.info("No live or upcoming matches to update. Exiting.")
            await pool.close()
            return
    ...
    log.info(f"Total live matches to update: {progress['total']}")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        workers = [asyncio.create_task(worker(pool, session, queue, i)) for i in range(10)]
        await queue.join()
    ...
    elapsed = time.time() - progress['start']
    log.info(f"Upsert Complete! Processed {progress['done']} matches in {elapsed/60:.1f} minutes")
