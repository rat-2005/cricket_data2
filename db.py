...
def sync_parquet_files(target_dir='data'):
    ...
    log.info(f"Syncing parquet files locally from S3 to {target_dir}...")
    import os
    ...
    try:
        ...
        log.info("Parquet sync complete.")
    except Exception as e:
        log.error(f"Failed to sync parquet files: {e}")
...
def safe_hot_swap():
    ...
    log.info("Starting safe hot-swap...")
    ...
    log.info("Swapped data folders. Reloading DuckDB...")
    ...
    if os.path.exists('data_old'):
        try:
            ...
            log.warning(f"Could not delete data_old: {e}")
            
    log.info("Hot-swap complete! Zero downtime.")
...
def get_conn():
    ...
    log.info("Initializing DuckDB with local Parquet access...")
    import time
    max_retries = 3
    for attempt in range(max_retries):
        ...
        try:
            ...
            _conn = conn
            log.info("DuckDB ready — all S3 views and lookup tables created.")
            return _conn

        except Exception as e:
            conn.close()
            log.error(f"DuckDB S3 connection failed (Attempt {attempt+1}/{max_retries}): {e}")
